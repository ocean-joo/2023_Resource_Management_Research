import math
from autoware_msgs.msg import LaneArray, NDTStat, VehicleCmd, VehicleStatus
...
def write_position_info():
    ...
    with open(center_offset_file, "w") as f:
        ...
        center_offset_wr.writerow(['ts', 'state', 'center_offset', 'ndt_response_time(ms)', 'instance', 'gnss_pose_x', 'gnss_pose_y', 'ndt_score', 'current_pose_x', 'current_pose_y', 'current_speed'])
        prev_dis = 0
        while not rospy.is_shutdown():
            ...
            status_msg = rospy.wait_for_message('/vehicle_status', VehicleStatus, timeout=None)
            ...
            current_speed = status_msg.speed

            center_offset_wr.writerow([time.clock_gettime(time.CLOCK_MONOTONIC), state_text, str(min_dis), str(ndt_stat_msg.exe_time), instance, gnss_pose_x, gnss_pose_y, str(ndt_stat_msg.score), current_pose_x, current_pose_y, current_speed])
    f.close()
